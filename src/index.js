module.exports = function(schema, option) {
  ...
  const importComponent = (type) => {
    ...
    if (componentsMap && !components.includes(type)) {
      ...
      if (componentMap) {
        const pkgName = componentMap.package ? componentMap.package : 'ant-design-vue';

        // hack
        componentMap.destructuring = 1;

        if (componentMap.exportName) {
          console.log(0,exportName)
          ...
          let exportName = componentMap.destructuring ? `{ ${prefix}${componentMap.name} }` : componentMap.name;
          result = componentMap.main
            ? `import ${exportName} from '${pkgName}${componentMap.main}'`
            : `import ${exportName} from '${pkgName}'`;
        }
        ...
        result = `import {${prefix}${type}} from 'ant-design-vue'`;
  ...
  const generateRender = (schema) => {
    ...
    switch (type) {
      ...
      default:
        ...
        // const prefix = ''
        if (schema.children && schema.children.length) {
          // xml = `<${prefix}${type.toLowerCase()}${classString}${props}>${transform(schema.children)}</${type}>`;
          xml = `<${type}${classString}${props}>${transform(schema.children)}</${type}>`;
        } else {
          // xml = `<${prefix}${type.toLowerCase()}${classString}${props} />`;
